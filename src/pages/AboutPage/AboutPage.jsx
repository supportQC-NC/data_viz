...
const MANGROVE_STATS = [
  { value: '40%', label: 'des mangroves mondiales perdues', sub: 'depuis 1980', color: '#FF6B35', icon: '🌿' },
  { value: '3.2M', label: 'hectares dans le Pacifique', sub: 'surface restante surveillée', color: '#00E5C3', icon: '🛰️' },
  { value: '70×', label: 'plus efficaces que les forêts', sub: 'pour le stockage de carbone', color: '#69F0AE', icon: '🌱' },
  { value: '1Md', label: 'personnes protégées', sub: 'des submersions côtières', color: '#40C4FF', icon: '👥' },
];
...
const TEAM_DATA = [
  { name: 'Données satellites', source: 'JAXA ALOS-2', color: '#40C4FF', icon: '🛰️' },
  { name: 'Niveaux marins', source: 'NOAA Tide Gauges', color: '#00E5FF', icon: '🌊' },
  { name: 'Projections IPCC', source: 'AR6 — SSP1-2.6 / SSP5-8.5', color: '#69F0AE', icon: '📊' },
  { name: 'Mangroves', source: 'Global Mangrove Watch', color: '#00E5C3', icon: '🌿' },
  { name: 'Cyclones', source: 'IBTrACS NOAA', color: '#FF6D00', icon: '🌀' },
  { name: 'Populations', source: 'CoastalDEM v2', color: '#FF5252', icon: '👥' },
];
...
const AnimatedStat = ({ stat, index }) => {
  ...
  return (
    ...
    >
      ...
      <div className="about__stat-glow" style={{ background: `radial-gradient(circle, ${stat.color}15, transparent 70%)` }} />
    </div>
  );
};
...
const TimelineItem = ({ item, index, isLast }) => {
  ...
  return (
    ...
    >
      ...
      >
        <div className="about__tl-year" style={{ color: item.color }}>{item.year}</div>
        <div className="about__tl-title">{item.title}</div>
        <div className="about__tl-desc">{item.desc}</div>
        <div className="about__tl-sea" style={{ borderColor: `${item.color}50`, color: item.color }}>
  ...
  return (
    <div className="about">
      ...
      <nav className="about__nav">
        <button type="button" className="about__nav-logo" onClick={() => navigate('/')}>
          🌊 PacificShield
        </button>

        <div className="about__nav-links">
          <button type="button" onClick={() => navigate('/map')} className="about__nav-btn">
            Explorer la carte →
          </button>
        </div>
      </nav>
      ...
      <section className="about__hero">
        <div className="about__hero-content" style={parallaxStyle}>
          ...
          <p className="about__hero-lead">
            Dans le Pacifique, une armée silencieuse de mangroves lutte depuis des millénaires contre la montée des eaux.
            Nous la perdons à un rythme alarmant.
          </p>
      ...
      <section className="about__manifesto">
        <div className="about__manifesto-inner">
          {[
            ['#00E5FF', 'PacificShield est né d’une conviction : les données climatiques doivent être visibles, tangibles, émotionnelles.'],
            ['#69F0AE', 'Chaque mangrove détruite est une digue naturelle perdue. Chaque millimètre de montée des eaux est une réalité vécue.'],
            ['#FF6B35', 'Nous avons cartographié l’invisible pour rendre l’urgence impossible à ignorer.'],
          ].map(([color, text]) => (
            <div key={text} className="about__manifesto-line">
              <div className="about__manifesto-bar" style={{ background: color }} />
              <p><strong style={{ color }}>PacificShield</strong> — {text}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="about__stats">
        <div className="about__section-header">
          ...
          <p className="about__section-sub">
            Les mangroves stockent du carbone, filtrent l’eau, protègent les côtes et forment un rempart naturel contre les cyclones.
          </p>
        </div>
        ...
        <div className="about__mangrove-diagram">
          <div className="about__diag-title">Pourquoi les mangroves sont irremplaçables</div>

          <div className="about__diag-items">
            {[
              { icon: '💨', label: 'Brise-vent', desc: 'Réduit la force des cyclones' },
              { icon: '🌊', label: 'Atténuation', desc: 'Absorbe les vagues de submersion' },
              { icon: '🐟', label: 'Nurserie', desc: 'Protège les espèces marines' },
              { icon: '🌍', label: 'Carbone bleu', desc: 'Stocke énormément de carbone' },
              { icon: '💧', label: 'Épuration', desc: 'Filtre les polluants côtiers' },
      ...
      <section className="about__timeline">
        <div className="about__section-header">
          ...
          <p className="about__section-sub">
            De la déforestation aux projections de 2100, la crise climatique du Pacifique s’est construite progressivement.
          </p>
        </div>
      ...
      <section className="about__zones">
        <div className="about__section-header">
          ...
          <h2 className="about__section-title">Les nations en première ligne</h2>
        </div>

        <div className="about__zones-grid">
          {[
            { flag: '🇹🇻', name: 'Tuvalu', stat: '2m max', detail: 'Plan d’évacuation en cours', urgency: 'CRITIQUE', color: '#FF1744' },
            { flag: '🇰🇮', name: 'Kiribati', stat: '56 388 hab.', detail: 'Population fortement exposée', urgency: 'CRITIQUE', color: '#FF5252' },
            { flag: '🇻🇺', name: 'Vanuatu', stat: '#1 mondial', detail: 'Très forte vulnérabilité climatique', urgency: 'DANGER', color: '#FF6D00' },
            { flag: '🇳🇨', name: 'Nouvelle-Calédonie', stat: '62 000', detail: 'Zones côtières menacées', urgency: 'MENACÉ', color: '#FFD740' },
            { flag: '🇫🇯', name: 'Fidji', stat: '2× cat.5', detail: 'Cyclones majeurs récents', urgency: 'DANGER', color: '#FF9100' },
            { flag: '🇸🇧', name: 'Îles Salomon', stat: '5 îles', detail: 'Îles déjà fortement touchées', urgency: 'DANGER', color: '#FF6B35' },
          ].map((zone) => (
            <div
              key={zone.name}
              ...
              <div className="about__zone-bar">
                <div
                  ...
                  style={{
                    ...
                    width: zone.urgency === 'CRITIQUE' ? '90%' : zone.urgency === 'DANGER' ? '65%' : '40%',
                  }}
                />
              </div>
            </div>
      ...
      <section className="about__data">
        <div className="about__section-header">
          ...
          <h2 className="about__section-title">Science ouverte, données satellites</h2>
          <p className="about__section-sub">
            PacificShield agrège des données open-source et prépare l’intégration des datasets officiels.
          </p>
        </div>
        ...
        <div className="about__data-grid">
          {TEAM_DATA.map((item) => (
            ...
            >
              <div className="about__data-icon" style={{ color: item.color }}>{item.icon}</div>
              <div className="about__data-name" style={{ color: item.color }}>{item.name}</div>
        ...
        <div className="about__arch-note">
          ...
          <div>
            ...
            <div className="about__arch-desc">
              Toutes les couches de données sont abstraites en interfaces standardisées pour faciliter le remplacement des données mockées.
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="about__cta">
        <div className="about__cta-inner">
          ...
          <p className="about__cta-sub">
            Simulez la montée des eaux, identifiez les zones de mangroves et visualisez ce qui peut encore être sauvé.
          </p>

          <button type="button" className="about__cta-btn" onClick={() => navigate('/map')}>
            <span>Lancer l’expérience</span>
            <span className="about__cta-arrow">→</span>
          </button>

          <div className="about__cta-credits">
            Pacific Dataviz Challenge 2026 · Données : NOAA · NASA · JAXA · SPREP · IPCC AR6
          </div>
        </div>
      </section>
    </div>
