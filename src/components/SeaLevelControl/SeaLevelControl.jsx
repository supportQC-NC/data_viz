...
const UI_MAX = 12;
const lerp = (a, b, t) => a + (b - a) * t; // eslint-disable-line no-unused-vars
...
const PACIFIC_FACTS = [
  {
    icon: "🌊",
    val: "+5.8mm/an",
    label: "Montée annuelle Pacifique Sud",
    color: "#00E5FF",
  },
  {
    icon: "🏝️",
    val: "5 îles",
    label: "Îles Salomon englouties (2016)",
    color: "#FF5252",
  },
  {
    icon: "🪸",
    val: "−78%",
    label: "Coraux blanchis Pacifique 2024",
    color: "#FF9100",
  },
  {
    icon: "🌀",
    val: "×1.8",
    label: "Cyclones cat.5 vs décennie 80",
    color: "#FFD740",
  },
];
...
const getPopAtRisk = (r) => {
  ...
  for (let i = 0; i < pts.length - 1; i++) {
    const [a, va] = [pts[i][0], pts[i][1]],
      [b, vb] = [pts[i + 1][0], pts[i + 1][1]];
    if (r >= a && r <= b)
      return Math.round(va + ((r - a) / (b - a)) * (vb - va));
  }
  return 1100e6;
};
...
const getClosest = (r) =>
  SEA_LEVEL_SCENARIOS.reduce((p, c) =>
    Math.abs(c.rise - r) < Math.abs(p.rise - r) ? c : p,
  );
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (let i = 0; i < 25; i++) {
        ...
        ctx.beginPath();
        ctx.arc(sx, sy, 0.6 + (i % 3) * 0.25, 0, Math.PI * 2);
      ...
      hg.addColorStop(1, `${color}18`);
      ctx.fillStyle = hg;
      ctx.fillRect(0, wY - 20, W, 30);
      ...
      ctx.lineTo(W, H);
      ctx.closePath();
      ctx.fillStyle = `rgba(12,40,18,${islandAlpha})`;
      ctx.fill();
      ...
      if (norm < 0.6) {
        ...
        [0.18, 0.38, 0.58, 0.76].forEach((xr, i) => {
          const mx = xr * W,
            my = wY + (i % 2 ? 2 : -2);
          ctx.beginPath();
          ctx.arc(mx, my - 9, 7 * (1 - norm * 0.6), 0, Math.PI * 2);
          ctx.fillStyle = `rgba(10,55,18,${ma * 0.9})`;
          ctx.fill();
          ctx.beginPath();
          ctx.moveTo(mx, my);
          ctx.lineTo(mx, my - 7);
          ctx.strokeStyle = `rgba(8,45,14,${ma})`;
          ctx.lineWidth = 2;
          ctx.stroke();
        });
      }
      ...
      [
        ...
        for (let x = 0; x <= W; x += 2) {
          ...
          const y =
            wY +
            Math.sin(r * 4 * Math.PI + time * spd) * a2 +
            Math.sin(r * 7 * Math.PI + time * spd * 0.7) * a2 * 0.3;
          ctx.lineTo(x, y);
        }
        ctx.lineTo(W, H);
        ctx.closePath();
        ctx.fillStyle = `rgba(${r2},${g2},${b2},${opa})`;
        ctx.fill();
      });
      ...
      wg.addColorStop(
        1,
        `rgba(${Math.max(0, r2 - 10)},${Math.max(0, g2 - 15)},${Math.max(0, b2 - 20)},0.92)`,
      );
      ctx.fillStyle = wg;
      ctx.fillRect(0, wY + 12, W, H);
  ...
  return (
    <div className={`slc__ncz${sub ? " slc__ncz--sub" : ""}`}>
      <div className="slc__ncz-head">
        ...
        <span
          className="slc__ncz-status"
          style={{ color: sub ? "#FF1744" : par ? "#FF9100" : "#43A047" }}
        >
      ...
      <div className="slc__ncz-meta">
        {(zone.pop / 1000).toFixed(0)}k hab · risque à +{zone.riseAt}m
      </div>
    </div>
  );
};
...
const SeaLevelControl = ({
  ...
  const dangerCol =
    vr < 0.5 ? "#69F0AE" : vr < 2 ? "#FFD740" : vr < 5 ? "#FF9100" : "#FF1744";
      ...
      <div className="slc__panel">
        ...
        <div className="slc__header" style={{ "--c": color }}>
          <div className="slc__header-left">
            <div className="slc__hdr-label" style={{ color }}>
              {closest.label}
            </div>
        ...
        {!collapsed && (
          <>
                  ...
                  >
                    <div
                      ...
                      style={{
                        ...
                        boxShadow:
                          s.rise <= seaRise ? `0 0 5px ${s.color}` : "none",
                      }}
                    />
                    <div
                      className="slc__tick-lbl"
                      style={{
                        color:
                          s.rise <= seaRise ? s.color : "rgba(255,255,255,0.3)",
                      }}
                    >
                      {s.shortLabel}
                    </div>
            ...
            <div className="slc__scenarios">
              {uiScenarios.map((s) => {
                ...
                return (
                  ...
                  >
                    <div
                      className="slc__sc-dot"
                      style={{
                        background: s.color,
                        boxShadow: active ? `0 0 7px ${s.color}` : "none",
                      }}
                    />
                    <div className="slc__sc-info">
                      <div
                        className="slc__sc-name"
                        style={{
                          color: active ? s.color : "rgba(255,255,255,0.8)",
                        }}
                      >
                        {s.shortLabel}
                      </div>
                      {s.ipccScenario && (
                        <div className="slc__sc-code">{s.ipccScenario}</div>
                      )}
                    </div>
                    <div
                      className="slc__sc-rise"
                      style={{
            ...
            <div className="slc__body">
              {tab === "sim" && (
                <div className="slc__stats">
                  ...
                  {[
                    [
                      "👥",
                      "Population mondiale à risque",
                      fmtPop(pop),
                      dangerCol,
                    ],
                    [
                      "🌡",
                      "Anomalie temp. (vs préind.)",
                      closest.tempAnomaly != null
                        ? `+${closest.tempAnomaly.toFixed(1)} °C`
                        : "—",
                      closest.tempAnomaly > 4
                        ? "#FF1744"
                        : closest.tempAnomaly > 2
                          ? "#FF9100"
                          : "#FFD740",
                    ],
                    [
                      "🧊",
                      "Fonte glaces arctiques",
                      closest.arcticIceLoss != null
                        ? `−${closest.arcticIceLoss}%`
                        : "—",
                      "#80D8FF",
                    ],
                    [
                      "🌿",
                      "Mangroves Pac. perdues",
                      vr < 0.5
                        ? "−8%"
                        : vr < 2
                          ? "−22%"
                    ...
                    <div key={lbl} className="slc__row">
                      ...
                      <span className="slc__row-val" style={{ color: vc }}>
                        {val}
                      </span>
                    </div>
                  ))}
                ...
                <div className="slc__nc">
                  <div className="slc__nc-top">
                    ...
                    <div className="slc__nc-meta">
                      ...
                      <span style={{ color: "#29B6F6", fontWeight: 700 }}>
                        +{NC_DATA.seaRise.median2100}m
                      </span>
                    </div>
                    <div className="slc__nc-meta">
                      <span>Tendance observée</span>
                      <span style={{ color: "#FF9100", fontWeight: 700 }}>
                        {NC_DATA.seaRise.observed}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="slc__nc-zones">
                    {NC_DATA.zones.map((z) => (
                      <NCZoneBar key={z.name} zone={z} seaRise={vr} />
                    ))}
                  </div>
                  ...
                  <div className="slc__nc-mang">
                    ...
                    <div className="slc__nc-mang-row">
                      {[
                        [
                          NC_DATA.mangroves.totalHa.toLocaleString("fr-FR") +
                            " ha",
                          "Surface totale",
                          "#4CAF50",
                        ],
                  ...
                  <div className="slc__nc-coral">
                    <div className="slc__nc-mang-ttl">
                      🪸 Récifs coralliens NC
                    </div>
                    <div className="slc__nc-mang-row">
                      {[
                        [
                          NC_DATA.coral.coveragePct + "%",
                          "Couverture vivante",
                          "#00E5FF",
                        ],
                        [
                          NC_DATA.coral.bleachingEvents + "×",
                          "Événements blanchissement",
                          "#FF9100",
                        ],
                        [
                          NC_DATA.coral.mpaPct + "%",
                  ...
                  <div className="slc__alert">
                    ⚠ Ouvéa submergée à +1m · Plan DIMENC avant 2040 ·{" "}
                    {NC_DATA.zones.filter((z) => vr >= z.riseAt).length} zone
                    {NC_DATA.zones.filter((z) => vr >= z.riseAt).length > 1
                      ? "s"
                      : ""}{" "}
                    à risque actuellement
                  </div>
                </div>
              )}
              ...
              {tab === "noaa" && (
                <div className="slc__noaa">
                  {[
                    {
                      ...
                      content: (
                        <select
                          className="slc__select"
                          value={noaaStationId}
                          onChange={(e) =>
                            onNoaaStationChange?.(e.target.value)
                          }
                        >
                          {noaaStations.map((s) => (
                    ...
                    {
                      ico: "🌊",
                      lbl: "Niveau observé (MSL)",
                      content: (
                        <div
                          className="slc__noaa-val"
                          style={{ color: obs == null ? "#FFD740" : "#00E5FF" }}
                        >
                          {noaaLoading
                            ? "⏳ Chargement…"
                            : obs == null
                              ? "— indisponible"
                              : `${obs >= 0 ? "+" : ""}${obs.toFixed(3)} m`}
                        </div>
                      ),
                    },
                    {
                      ...
                      content: (
                        ...
                        >
                          {noaaLevel?.time
                            ? new Date(
                                noaaLevel.time.includes("T")
                                  ? noaaLevel.time
                                  : noaaLevel.time.replace(" ", "T") + "Z",
                  ...
                  <label className="slc__noaa-row slc__noaa-row--toggle">
                    ...
                    <div className="slc__toggle">
                      <input
                        ...
                        onChange={(e) =>
                          onIncludeNoaaLevelChange?.(e.target.checked)
                        }
                      />
                      <label
                        htmlFor="noaa-toggle"
                        className="slc__toggle-track"
                      >
                  ...
                  {noaaError && (
                    <div className="slc__noaa-err">⚠ {noaaError}</div>
                  )}
                </div>
              )}
            </div>
            ...
            />
            <div className="slc__credit">
              IPCC AR6 · NOAA CO-OPS · NASA · SPREP · DIMENC · IRD
            </div>
          </>
        )}
        ...
        {collapsed && (
          <div className="slc__collapsed-summary">
            ...
            <div className="slc__cs-row">
              ...
              >
                {closest.ipccScenario || "Référence"}
              </span>
              <span
