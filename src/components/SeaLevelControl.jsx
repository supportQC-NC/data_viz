...
const getPopAtRisk = (rise) => {
  ...
  for (let i = 0; i < pts.length - 1; i += 1) {
    ...
    if (rise >= a && rise <= b) {
      return Math.round(va + ((rise - a) / (b - a)) * (vb - va));
    }
  }
...
const getInterp = (rise) => {
  for (let i = 0; i < SEA_LEVEL_SCENARIOS.length - 1; i += 1) {
    const a = SEA_LEVEL_SCENARIOS[i];
    const b = SEA_LEVEL_SCENARIOS[i + 1];
    ...
    if (rise >= a.rise && rise <= b.rise) {
      ...
      return {
        waveIntensity: (a.waveIntensity || 0.15) + t * ((b.waveIntensity || 1) - (a.waveIntensity || 0.15)),
        tempAnomaly:
          a.tempAnomaly != null && b.tempAnomaly != null
...
const SeaLevelControl = ({
  ...
  const visualRise = Number.isFinite(effectiveSeaRise) ? effectiveSeaRise : seaRise;

  const closest = useMemo(() => getClosest(visualRise), [visualRise]);
  const interp = useMemo(() => getInterp(visualRise), [visualRise]);
  const pop = useMemo(() => getPopAtRisk(visualRise), [visualRise]);

  const color = closest.color || '#00e5ff';
  const pct = Math.min(seaRise / UI_MAX, 1) * 100;

  const dangerCol =
    visualRise < 0.5 ? '#69f0ae'
      : visualRise < 2 ? '#ffd740'
        : visualRise < 5 ? '#ffab40'
          : '#ff5252';
  ...
  const observedValue = Number.isFinite(noaaLevel?.value) ? noaaLevel.value : null;
    ...
    <div className="slc">
      <div className="slc__panel">
        <div className="slc__head">
          ...
          <div className="slc__badge">
            <div
              className="slc__badge-num"
              style={{
                color,
                textShadow: `0 0 28px ${color}`,
              }}
            >
              +{visualRise < 10 ? visualRise.toFixed(2) : visualRise.toFixed(1)}
              <span className="slc__badge-unit">m</span>
            </div>
        ...
        <div className="slc__body">
          <div className="slc__wbar">
            <div
              className="slc__wbar-fill"
              style={{
                height: `${Math.min(visualRise / UI_MAX, 1) * 100}%`,
                background: `linear-gradient(to top, ${color}cc, ${color}40)`,
                color,
              }}
            />

            <div className="slc__wbar-label">
              Simulation + donnée NOAA
            </div>
          </div>
            ...
            {SEA_LEVEL_SCENARIOS.filter((s) => s.rise <= UI_MAX).map((s) => {
              ...
              return (
                ...
                >
                  ...
                  <span
                    className="slc__scene__label"
                    style={{
                      color: active ? s.color : 'rgba(255,255,255,0.52)',
                    }}
                  >
                    {s.shortLabel}
                  </span>
          ...
          <div className="slc__stats">
            ...
            <div className="slc__stat">
              <span className="slc__stat-label">🌊 Simulation</span>
              <span className="slc__stat-value" style={{ color }}>
                +{seaRise.toFixed(2)} m
              </span>
            </div>

            <div className="slc__stat">
              <span className="slc__stat-label">📡 NOAA observé</span>
              <span className="slc__stat-value" style={{ color: observedValue == null ? '#ffab40' : '#80d8ff' }}>
                {noaaLoading
                  ? 'Chargement...'
                  : observedValue == null
                    ? '—'
                    : `${observedValue >= 0 ? '+' : ''}${observedValue.toFixed(2)} m`}
              </span>
            </div>

            <div className="slc__stat">
              <span className="slc__stat-label">🧮 Total carte</span>
              <span className="slc__stat-value" style={{ color }}>
                +{visualRise.toFixed(2)} m
              </span>
            </div>

            <div className="slc__stat">
              <span className="slc__stat-label">👥 Pop. à risque</span>
              <span className="slc__stat-value" style={{ color: dangerCol }}>
                {fmtPop(pop)}
              </span>
            </div>
            ...
            {interp.tempAnomaly != null && (
              <div className="slc__stat">
                <span className="slc__stat-label">🌡 Anomalie temp.</span>
                <span
                  className="slc__stat-value"
                  style={{
                    color:
                      interp.tempAnomaly > 4 ? '#ff1744'
                        : interp.tempAnomaly > 2 ? '#ff6d00'
                          : '#ffd740',
                  }}
                >
                  +{interp.tempAnomaly.toFixed(1)} °C
          ...
          <div className="slc__stats">
            <span className="slc__stats-title">Donnée réelle NOAA</span>

            <div className="slc__stat">
              <span className="slc__stat-label">📍 Station</span>

              <select
                value={noaaStationId}
                onChange={(e) => onNoaaStationChange?.(e.target.value)}
            ...
            <div className="slc__stat">
              <span className="slc__stat-label">🕒 Mesure</span>
              <span className="slc__stat-value" style={{ color: 'rgba(255,255,255,0.82)' }}>
                {formatNoaaDate(noaaLevel?.time)}
              </span>
            </div>

            <label className="slc__stat">
              <span className="slc__stat-label">➕ Ajouter à la carte</span>
            ...
            {selectedNoaaStation && (
              <div className="slc__desc">
                Station active : {selectedNoaaStation.label}.  
                La valeur NOAA correspond au niveau observé par rapport au datum MSL.
              </div>
            )}

            {noaaError && (
              <div className="slc__desc" style={{ color: '#ff8a80' }}>
                NOAA : {noaaError}
              </div>
            )}
          </div>
          ...
          <div className="slc__credit">
            Données simulation · IPCC AR6 · NOAA CO-OPS JSON · NASA · SPREP
          </div>
        </div>

        <div
          className="slc__foot-bar"
          style={{
            background: `linear-gradient(to right, transparent, ${color}, transparent)`,
          }}
        />
